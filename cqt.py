...
class CQT(data.Dataset):
    ...
    def __init__(self, filelist, trg_shift=1, block_size=1024,
        ...
        # # TODO: FORK - workers or not
        # print(f"doing forced files_to_cqt, '\n file list: {filelist}" )
    ...
    def process_file(self, file):
        ...
        for i in range(0, len(cqt), self.block_size):
            if self.one_shot and i > 0:
                break
            # TODO : MAJOR CHANGE
            # if not self.allow_diff_shapes and i + self.block_size < len(cqt):
            if not self.allow_diff_shapes and i + self.block_size > len(cqt):
                break
        ...
        # for i in range(0, len(cqt), block_size):
        #     if one_shot and i > 0:
        #         break
        #     if not allow_diff_shapes and i + block_size > \
        #             len(cqt):
        #         break
        #
        #     cqt_x_block = cqt_x[i:i + block_size]
        #     cqt_y_block = cqt_y[i:i + block_size]
        #
        #     length = len(cqt_x_block)
        #     lengths.append(length)
        #
        #     if padded:
        #         diff = block_size - length
        #         if diff > 0:
        #             pad = np.ones((diff, *cqt_x_block.shape[1:])) * \
        #                   cqt_x_block[-1]
        #             cqt_x_block = np.concatenate((cqt_x_block, pad))
